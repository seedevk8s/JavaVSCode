...
```mermaid
graph TD
    subgraph Heavy ["🐌 Model 직접 사용 - 무거운 응답"]
        HeavyModel[User Entity 전체<br/>• 기본 정보 (1KB)<br/>• orders 100개 (200KB)<br/>• reviews 200개 (150KB)<br/>• department 상세정보 (50KB)<br/>• 연관 테이블 데이터<br/>📊 총 크기: 500KB]
        SlowProcess[처리 과정<br/>🔄 모든 연관 데이터 로딩<br/>🔄 JOIN 쿼리 다수 실행<br/>🔄 대용량 JSON 직렬화<br/>⏱️ 데이터베이스: 3초<br/>⏱️ 직렬화: 2초]
        SlowResult[클라이언트 응답<br/>🐌 총 응답시간: 5초<br/>📶 네트워크 사용량: 500KB<br/>💾 모바일 데이터 소모 심각]
        
        HeavyModel ==> SlowProcess
        SlowProcess ==> SlowResult
    end
    
    subgraph Light ["🚀 DTO 사용 - 가벼운 응답"]
        LightDTO[UserListResponse DTO<br/>• id (필수)<br/>• name (필수)<br/>• email (필수)<br/>불필요한 데이터 제외<br/>📊 총 크기: 1KB]
        FastProcess[처리 과정<br/>✅ 필요한 필드만 SELECT<br/>✅ JOIN 최소화<br/>✅ 가벼운 JSON 직렬화<br/>⏱️ 데이터베이스: 0.05초<br/>⏱️ 직렬화: 0.05초]
        FastResult[클라이언트 응답<br/>🚀 총 응답시간: 0.1초<br/>📶 네트워크 사용량: 1KB<br/>💾 모바일 친화적]
        
        LightDTO ==> FastProcess
        FastProcess ==> FastResult
    end
    
    ComparisonResult[📊 성능 비교 결과<br/>⚡ 응답 속도: 50배 향상<br/>📶 데이터 사용량: 500배 절약<br/>💰 서버 비용: 대폭 절감<br/>📱 사용자 경험: 크게 개선]
    
    SlowResult -.->|vs| ComparisonResult
    FastResult -.->|vs| ComparisonResult
    
    classDef slowStyle fill:#e74c3c,stroke:#c0392b,stroke-width:3px,color:#fff
    classDef fastStyle fill:#27ae60,stroke:#229954,stroke-width:3px,color:#fff
    classDef compareStyle fill:#3498db,stroke:#2980b9,stroke-width:2px,color:#fff
    
    class HeavyModel,SlowProcess,SlowResult slowStyle
    class LightDTO,FastProcess,FastResult fastStyle
    class ComparisonResult compareStyle
```
